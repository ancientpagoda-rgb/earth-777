...
import { collapseObservedFaunaPlan } from "../sim/FaunaBehaviorDynamics.js";
import {
...
export class SurfaceFaunaManager {
  constructor(scene, terrain) {
    ...
    this.behaviorBand = null;
    this.memorySeed = null;
    this.aggregateMemory = new Map();
    this.lastCollapsedState = null;
    ...
    this.herdPool = new PagedInstancePool(scene, new THREE.SphereGeometry(0.0042, 6, 4), new THREE.MeshStandardMaterial({ color: 0x62503a, roughness: 1, transparent: true, opacity: 0.72 }), 256);
    this.packPool = new PagedInstancePool(scene, new THREE.SphereGeometry(0.0034, 6, 4), new THREE.MeshStandardMaterial({ color: 0x493a34, roughness: 1, transparent: true, opacity: 0.76 }), 128);
    this.individualPool = new PagedInstancePool(scene, new THREE.SphereGeometry(0.0012, 5, 3), new THREE.MeshStandardMaterial({ color: 0x4b3c2d, roughness: 1 }), 512);
    this.predatorPool = new PagedInstancePool(scene, new THREE.SphereGeometry(0.0010, 5, 3), new THREE.MeshStandardMaterial({ color: 0x342c29, roughness: 1 }), 256);
  ...
  _collapseObserved() {
    if (!this.observedPlan) return null;
    const collapsed = collapseObservedFaunaPlan(this.observedPlan);
    if (collapsed && this.observedPlan.focusKey) {
      this.aggregateMemory.set(this.observedPlan.focusKey, collapsed);
  ...
  setContext({ latitude, longitude, state, vegetationSample = null, hydrologySample = null } = {}) {
    const nextSeed = Number(state?.seed) >>> 0;
    if (this.memorySeed !== nextSeed) {
      this.aggregateMemory.clear();
      this.lastCollapsedState = null;
      this.memorySeed = nextSeed;
    }
    ...
    const nextBehaviorBand = Math.floor((Number(state?.elapsedYears) || 0) * 12);
    const structuralChanged = structuralSignature !== this.contextSignature;
    const behaviorChanged = nextBehaviorBand !== this.behaviorBand;
    this.context = { latitude: Number(latitude) || 0, longitude: Number(longitude) || 0, state, vegetationSample, hydrologySample };
    this.behaviorBand = nextBehaviorBand;
    if (!structuralChanged && !behaviorChanged) return false;
    if (this.observedPlan) this._collapseObserved();
    if (structuralChanged) {
      this.contextSignature = structuralSignature;
      this.regionalDirty = true;
      this.localDirty = true;
    }
    this.observedDirty = true;
  ...
  _prepareObserved() {
    ...
    const focus = this.terrain._geographicAt?.(this.cameraPosition.x, this.cameraPosition.z) ?? { latitude: this.context.latitude, longitude: this.context.longitude };
    const seed = this.context.state?.seed ?? this.terrain.branchSeed ?? 777001;
    const focusKey = `observed:${Number(seed) >>> 0}:${Math.round(focus.latitude * 10)}:${Math.round(focus.longitude * 10)}`;
    const aggregatePrior = this.aggregateMemory.get(focusKey) ?? null;
    ...
    this.predatorPool.clear();
    this.renderQueue = [];
    for (const herd of this.observedPlan.herds) if (herd.representation === "herd") this.renderQueue.push({ type: "herd", value: herd });
    for (const pack of this.observedPlan.packs) if (pack.representation === "pack") this.renderQueue.push({ type: "pack", value: pack });
    for (const individual of this.observedPlan.individuals) this.renderQueue.push({ type: individual.role === "carnivore" ? "predator" : "individual", value: individual });
    this.renderCursor = 0;
  ...
  pumpObserved(sliceMs = 0.75) {
    ...
    let packIndex = this.packPool.count;
    let individualIndex = this.individualPool.count;
    let predatorIndex = this.predatorPool.count;
    while (this.renderCursor < this.renderQueue.length && performance.now() - started < Math.max(0.05, sliceMs)) {
      ...
      if (item.type === "herd" || item.type === "pack") {
        ...
        const scale = clamp(0.8 + Math.log1p(group.population) * 0.36, 0.8, 2.8);
        const pool = item.type === "pack" ? this.packPool : this.herdPool;
        const index = item.type === "pack" ? packIndex : herdIndex;
        const sizeX = item.type === "pack" ? scale * 1.6 : scale * 2.3;
        const sizeY = item.type === "pack" ? scale * 0.62 : scale * 0.72;
        if (this._setTransform(pool, index, group.x, group.z, sizeX, sizeY, scale, group.behaviorState?.heading ?? 0)) {
          if (item.type === "pack") packIndex += 1;
          else herdIndex += 1;
        }
      } else {
        const animal = item.value;
        const pool = item.type === "predator" ? this.predatorPool : this.individualPool;
        const index = item.type === "predator" ? predatorIndex : individualIndex;
        const length = item.type === "predator" ? animal.scale * 1.55 : animal.scale * 1.8;
        if (this._setTransform(pool, index, animal.x, animal.z, length, animal.scale, animal.scale * 0.8, animal.yaw)) {
          if (item.type === "predator") predatorIndex += 1;
          else individualIndex += 1;
        }
      }
      workUnits += 1;
    }
  ...
  diagnostics() {
    ...
    const localEstimatedPacks = this.localSummaries.reduce((sum, cell) => sum + (cell.estimatedPacks ?? 0), 0);
    return Object.freeze({
      policy: FAUNA_HIERARCHY_POLICY,
      epistemicStatus: FAUNA_EPISTEMIC_STATUS,
      behaviorPolicy: this.observedPlan?.behaviorPolicy ?? null,
      behaviorEpistemicStatus: this.observedPlan?.behaviorEpistemicStatus ?? null,
      ...
      observedVisibleCarnivores: this.observedPlan?.visibleCarnivorePopulation ?? 0,
      observedHerds: this.observedPlan?.herds?.length ?? 0,
      observedPacks: this.observedPlan?.packs?.length ?? 0,
      materializedIndividuals: this.observedPlan?.totalMaterializedPopulation ?? 0,
      materializedHerbivores: this.observedPlan?.materializedPopulation ?? 0,
      materializedCarnivores: this.observedPlan?.materializedCarnivores ?? 0,
      aggregateOnlyPopulation: this.observedPlan?.aggregateOnlyPopulation ?? 0,
      aggregateOnlyCarnivorePopulation: this.observedPlan?.aggregateOnlyCarnivorePopulation ?? 0,
      behaviorCounts: this.observedPlan?.behaviorCounts ?? Object.freeze({}),
      aggregateMemoryCells: this.aggregateMemory.size,
      lastCollapsedState: this.lastCollapsedState,
      renderQueueRemaining: Math.max(0, this.renderQueue.length - this.renderCursor),
      herdInstances: this.herdPool.diagnostics(),
      packInstances: this.packPool.diagnostics(),
      individualInstances: this.individualPool.diagnostics(),
      predatorInstances: this.predatorPool.diagnostics(),
      windowRadiusKm: this.windowRadiusKm,
      individualRadiusKm: this.individualRadiusKm
    });
  }

  clear({ preserveAggregateMemory = true } = {}) {
    this._collapseObserved();
    this.regionalFields = Object.freeze([]);
    this.localSummaries = Object.freeze([]);
    this.observedPlan = null;
  ...
  dispose() {
    this.clear({ preserveAggregateMemory: false });
    this.herdPool.dispose();
    this.packPool.dispose();
    this.individualPool.dispose();
    this.predatorPool.dispose();
    this.context = null;
  }
}
