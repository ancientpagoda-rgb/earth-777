...
export function faunaPopulationAt({ state = {}, vegetationSample = null, hydrologySample = null, latitude = 0, longitude = 0, areaKm2 = 1, key = "fauna" } = {}) {
  ...
  const productivity = npp > 0 ? clamp(Math.log1p(npp) / Math.log1p(2400), 0.04, 1) : clamp((state.productivityIndex ?? 1) / 1.5, 0.04, 1);
  ...
  const herbivoreDensity = clamp(0.035 + 3.4 * herbivoreBiomass ** 0.72 * (0.24 + productivity * 0.76) * (0.52 + waterAccess * 0.48) * biomeFactor(Number(vegetationSample?.biomeCode)), 0, 40);
  ...
  return Object.freeze({ policy: FAUNA_POLICY, epistemicStatus: FAUNA_EPISTEMIC_STATUS, key, latitude: Number(latitude) || 0, longitude: Number(longitude) || 0, areaKm2: area, herbivoreDensityAnimalsPerKm2: herbivoreDensity, carnivoreDensityAnimalsPerKm2: carnivoreDensity, herbivorePopulation, carnivorePopulation, estimatedHerds: herbivorePopulation > 0 ? Math.max(1, Math.ceil(herbivorePopulation / meanHerdSize)) : 0, estimatedPacks: carnivorePopulation > 0 ? Math.max(1, Math.ceil(carnivorePopulation / meanPackSize)) : 0, meanHerdSize, meanPackSize, productivity, waterAccess, predatorPressure, aggregatePredationExposure, predationExposure, preyPressure, biomeCode: vegetationSample?.biomeCode ?? null });
}
...
export function faunaForCells(cells = [], context = {}) {
  return Object.freeze(cells.map((cell) => {
    ...
    return faunaPopulationAt({ state: cellContext.state ?? context.state, vegetationSample: cellContext.vegetationSample, hydrologySample: cellContext.hydrologySample, latitude: cell.latitude, longitude: cell.longitude, areaKm2: approximateCellAreaKm2(cell), key: cell.key });
  }));
}
...
export function faunaGroupBehaviorAt({ role = "herbivore", id = "group", elapsedYears = 0, field = {}, lineage = null } = {}) {
  ...
  if (!lineage) {
    ...
    if (role === "carnivore") {
      ...
      const threat = clamp01((field.predatorPressure ?? 0.1) * 0.62 + (field.predationExposure ?? 0) * 0.10 + random01(hashText(id) + Math.floor(elapsedYears * 12)) * 0.18);
      threatIndex = threat;
      const need = clamp01((1 - (field.productivity ?? 0.6)) * 0.58 + (1 - (field.waterAccess ?? 0.6)) * 0.42);
      behavior = threat > 0.66 ? "flee" : need > 0.58 ? "travel" : phase < 0.15 ? "drink" : phase > 0.84 ? "rest" : "graze";
    }
    ...
    const distanceKm = behavior === "flee" ? 0.16 : behavior === "travel" || behavior === "hunt" ? 0.11 : behavior === "stalk" ? 0.06 : behavior === "drink" ? 0.04 : 0.025;
    return Object.freeze({ role, behavior, heading, distanceKm, threatIndex, preyPursuitDrive, resourceNeed: null, plantResourceNeed: null, preyResourceNeed: null, waterNeed: null, locomotionDrive: null, plantFeedingShare: null, livePreyFeedingShare: null });
  }
  const traits = lineageTraits(lineage);
  const feeding = feedingProfileForLineage(lineage);
  const primary = primaryFeedingShares(lineage, role);
  ...
  const rawThreat = clamp01((field.predatorPressure ?? 0.1) * 0.58 + (field.predationExposure ?? 0) * 0.12 + random01(hashText(id) + Math.floor(elapsedYears * 12)) * 0.18 + traits.cognition * 0.05 - traits.sociality * 0.05);
  ...
  return Object.freeze({ role, behavior, heading, distanceKm, threatIndex, preyPursuitDrive, resourceNeed, plantResourceNeed, preyResourceNeed, waterNeed, locomotionDrive, plantFeedingShare: primary.plantShare, livePreyFeedingShare: primary.livePreyShare });
}

function visiblePopulation(density, radiusKm) { return Math.max(0, Math.round(Math.max(0, density) * Math.PI * radiusKm * radiusKm)); }
...
function buildGroups({ role, population, meanSize, radiusKm, individualRadiusKm, focusXKm, focusZKm, seed, field, elapsedYears, lineages, state }) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    groups.push(Object.freeze({ id, role, lineageId: lineage?.id ?? null, population: groupPopulation, x, z, radiusKm: groupRadiusKm, representation: materialized ? "individuals" : role === "carnivore" ? "pack" : "herd", behavior: behavior.behavior, heading: behavior.heading, movementDistanceKm: behavior.distanceKm, threatIndex: behavior.threatIndex, preyPursuitDrive: behavior.preyPursuitDrive, resourceNeed: behavior.resourceNeed, plantResourceNeed: behavior.plantResourceNeed, preyResourceNeed: behavior.preyResourceNeed, waterNeed: behavior.waterNeed, locomotionDrive: behavior.locomotionDrive, plantFeedingShare: behavior.plantFeedingShare, livePreyFeedingShare: behavior.livePreyFeedingShare, localSuitability, channelAffinity, predationAffinity, plantMatterAffinity: feeding?.plantMatterAffinity ?? null, livePreyAffinity: feeding?.livePreyAffinity ?? null, carrionAffinity: feeding?.carrionAffinity ?? null, groupSizeScale: lineage ? groupSizeScale : null, spacingScale: lineage ? spacingScale : null, mobility: lineage ? traits.mobility : null, sociality: lineage ? traits.sociality : null, dietBreadth: lineage ? traits.dietBreadth : null, cognition: lineage ? traits.cognition : null, threatPerceptionRadiusKm: lineage ? herdThreatPerceptionRadiusKm({ mobility: traits.mobility, sociality: traits.sociality, cognition: traits.cognition }) : role === "herbivore" ? herdThreatPerceptionRadiusKm({ mobility: traits.mobility, sociality: traits.sociality, cognition: traits.cognition }) : null, bodyMassLog10Kg: lineage?.bodyMassLog10Kg ?? null, trophicLevel: feeding?.trophicLevel ?? lineage?.trophicLevel ?? null }));
    ...
    for (let animalIndex = 0; animalIndex < groupPopulation; animalIndex += 1) {
      ...
      individuals.push(Object.freeze({ id: `${id}:${animalIndex}`, groupId: id, role, lineageId: lineage?.id ?? null, behavior: behavior.behavior, x: x + Math.cos(animalAngle) * animalDistance + Math.cos(behavior.heading) * wobble, z: z + Math.sin(animalAngle) * animalDistance + Math.sin(behavior.heading) * wobble, yaw: behavior.heading, scale: baseScale * visualScale }));
    }
  }
...
function predatorPreyScore(pack, herd) {
  ...
  const massGap = Number.isFinite(predatorMass) && Number.isFinite(preyMass) ? Math.abs((preyMass - predatorMass) - 0.45) : 0;
  const sizeCompatibility = Math.exp(-0.55 * massGap * (1.05 - dietBreadth * 0.45));
  return Math.sqrt(Math.max(1, Number(herd.population) || 1)) * sizeCompatibility / (0.08 + distanceKm);
}

function predatorPerceptionRadiusKm(pack) { return clamp(0.38 + clamp01(pack?.cognition ?? 0.5) * 2.85 + clamp01(pack?.mobility ?? 0.5) * 0.42, 0.38, 3.65); }
...
function targetPredatorPacks(packs, herds) {
  ...
  return Object.freeze(packs.map((pack) => {
    ...
    for (const herd of herds) {
      ...
      if (score > bestScore) { bestScore = score; target = herd; }
    }
    ...
    return Object.freeze({ ...pack, x, z, heading, perceptionRadiusKm, preyPursuitDrive, targetGroupId: target.id, targetLineageId: target.lineageId ?? null, targetDistanceKm: Math.hypot(target.x - x, target.z - z), targetDistanceBeforeKm, approachDistanceKm, approachXKm, approachZKm });
  }));
}
...
function respondTargetedHerds(herds, packs) {
  ...
  return Object.freeze(herds.map((herd) => {
    ...
    for (let index = 1; index < threats.length; index += 1) {
      ...
      if (distanceKm < threatDistanceBeforeKm) { nearest = candidate; threatDistanceBeforeKm = distanceKm; }
    }
    ...
    const directThreatDrive = clamp01(proximityDrive * 0.65 + pursuitDrive * 0.25 + clamp01(threats.length / 3) * 0.10);
    const responseLocomotionDrive = hasContinuousLocomotion ? clamp01(Math.max(herd.locomotionDrive, directThreatDrive)) : null;
    const mobilityScale = 0.70 + clamp01(herd.mobility ?? 0.5) * 0.70;
    const strideRangeKm = 0.135 - clamp01(herd.predationAffinity ?? 0) * 0.05;
    const continuousResponseDistanceKm = hasContinuousLocomotion ? (0.025 + responseLocomotionDrive * strideRangeKm) * mobilityScale : 0;
    const fleeDistanceKm = hasContinuousLocomotion ? Math.max(Math.max(0, Number(herd.movementDistanceKm) || 0), continuousResponseDistanceKm) : Math.max(0, Number(herd.movementDistanceKm) || 0);
    ...
    return Object.freeze({ ...herd, x, z, behavior: "flee", heading, threatGroupId: nearest.id, threatLineageId: nearest.lineageId ?? null, threatDistanceKm: Math.hypot(x - nearest.x, z - nearest.z), threatDistanceBeforeKm, threatPerceptionRadiusKm: perceptionRadiusKm, directThreatDrive, responseLocomotionDrive, threatenedByCount: threats.length, fleeDistanceKm, fleeXKm, fleeZKm });
  }));
}
...
function spreadHerdAlarms(herds, packs) {
  ...
  return Object.freeze(herds.map((herd) => {
    ...
    for (const candidate of directlyThreatened) {
      ...
      const distanceKm = Math.hypot(herd.x - candidate.x, herd.z - candidate.z);
      if (distanceKm <= radiusKm && distanceKm < alarmDistanceKm) { source = candidate; alarmDistanceKm = distanceKm; alarmRadiusKm = radiusKm; }
    }
    ...
    return Object.freeze({ ...herd, x, z, behavior: "flee", heading, alarmSourceGroupId: source.id, alarmThreatGroupId: threat.id, alarmThreatLineageId: threat.lineageId ?? null, alarmDistanceKm, alarmRadiusKm, alarmResponseStrength: responseStrength, alarmThreatDistanceBeforeKm, alarmThreatDistanceKm: Math.hypot(x - threat.x, z - threat.z), alarmMoveDistanceKm, alarmMoveXKm, alarmMoveZKm });
  }));
}
...
function refreshPredatorTargetDistances(packs, herds) {
  ...
  return Object.freeze((packs ?? []).map((pack) => {
    ...
    return Object.freeze({ ...pack, targetDistanceAfterHerdResponseKm: finalDistanceKm, encounterClearanceKm, netClosingDistanceKm: (Number(pack.targetDistanceBeforeKm) || finalDistanceKm) - finalDistanceKm, encounterContact: finalDistanceKm <= encounterClearanceKm + 1e-12, encounterContactMarginKm: encounterClearanceKm - finalDistanceKm });
  }));
}
...
export function deriveEncounterEcologyProposal({ packs = [], herds = [], visiblePopulation = 0, visibleCarnivorePopulation = 0 } = {}) {
  ...
  for (const pack of packs) {
    ...
    const current = contactsByHerd.get(herd.id) ?? { herd, contactCount: 0, combinedPressure: 0, contactPacks: [] };
  ...
  const contactedHerds = Object.freeze([...contactsByHerd.values()].map(({ herd, contactCount: herdContactCount, combinedPressure, contactPacks }) => Object.freeze({ herdGroupId: herd.id, preyLineageId: herd.lineageId ?? null, observedPopulation: Math.max(0, Number(herd.population) || 0), observedPopulationFraction: totalHerbivores > 0 ? clamp01((Number(herd.population) || 0) / totalHerbivores) : 0, contactCount: herdContactCount, pressureIndex: clamp01(combinedPressure), contactPackIds: Object.freeze(contactPacks) })));
  return Object.freeze({ policy: ENCOUNTER_ECOLOGY_POLICY, epistemicStatus: "provisional geometric encounter pressure proposal; diagnostic only and not an aggregate population writer", authoritative: false, application: "not applied: aggregate ecology must consume equivalent spatially scheduled forcing independently of observation", contactCount, contactedHerdCount: contactedHerds.length, observedHerbivoreExposureIndex: clamp01(contactedHerds.reduce((sum, herd) => sum + herd.observedPopulationFraction * herd.pressureIndex, 0)), observedCarnivoreEngagementIndex: totalCarnivores > 0 ? clamp01(contactingCarnivorePopulation / totalCarnivores) : 0, contactedHerds });
}
...
function alignHerbivoreIndividuals(individuals, herds) {
  ...
  return Object.freeze((individuals ?? []).map((animal) => {
    ...
    return Object.freeze({ ...animal, behavior: "flee", x: animal.x + (Number(herd.fleeXKm) || Number(herd.alarmMoveXKm) || 0), z: animal.z + (Number(herd.fleeZKm) || Number(herd.alarmMoveZKm) || 0), yaw: herd.heading, threatGroupId: herd.threatGroupId ?? null, threatLineageId: herd.threatLineageId ?? null, alarmSourceGroupId: herd.alarmSourceGroupId ?? null, alarmThreatGroupId: herd.alarmThreatGroupId ?? null, alarmThreatLineageId: herd.alarmThreatLineageId ?? null }));
  }));
}
...
function alignCarnivoreIndividuals(individuals, packs) {
  ...
  return Object.freeze((individuals ?? []).map((animal) => {
    ...
    return Object.freeze({ ...animal, x: animal.x + (Number(pack.approachXKm) || 0), z: animal.z + (Number(pack.approachZKm) || 0), yaw: pack.heading, targetGroupId: pack.targetGroupId, targetLineageId: pack.targetLineageId ?? null });
  }));
}

export function buildObservedFauna({ state = {}, vegetationSample = null, hydrologySample = null, faunaField = null, latitude = 0, longitude = 0, seed = 777001, focusXKm = 0, focusZKm = 0, windowRadiusKm = 3.5, individualRadiusKm = 0.55 } = {}) {
  ...
  const field = faunaField ?? faunaPopulationAt({ state, vegetationSample, hydrologySample, latitude, longitude, areaKm2: Math.PI * radiusKm * radiusKm, key: "observed-window" });
  ...
  const encounterEcology = deriveEncounterEcologyProposal({ packs, herds, visiblePopulation: herbivorePopulation, visibleCarnivorePopulation: carnivorePopulation });
  return Object.freeze({ policy: FAUNA_POLICY, epistemicStatus: FAUNA_EPISTEMIC_STATUS, elapsedYears, field, windowRadiusKm: radiusKm, individualRadiusKm: nearRadiusKm, visiblePopulation: herbivorePopulation, visibleCarnivorePopulation: carnivorePopulation, herds, packs, individuals, lineageIds: Object.freeze([...new Set(groups.map((group) => group.lineageId).filter((id) => id != null))]), predatorTargetCount: packs.filter((pack) => pack.targetGroupId != null).length, threatenedHerdCount: herds.filter((herd) => herd.threatGroupId != null).length, alarmedHerdCount: herds.filter((herd) => herd.alarmThreatGroupId != null).length, movedAlarmedHerdCount: herds.filter((herd) => Number(herd.alarmMoveDistanceKm) > 0).length, predatorContactCount: packs.filter((pack) => pack.encounterContact === true).length, predatorGainCount: packs.filter((pack) => pack.targetGroupId != null && Number(pack.netClosingDistanceKm) > 0).length, encounterEcology, behaviorCounts: Object.freeze(behaviorCounts), materializedHerbivores: herbivoreIndividuals.length, materializedCarnivores: carnivoreIndividuals.length, totalMaterializedPopulation: individuals.length, aggregateOnlyPopulation: herds.filter((group) => group.representation === "herd").reduce((sum, group) => sum + group.population, 0), aggregateOnlyCarnivorePopulation: packs.filter((group) => group.representation === "pack").reduce((sum, group) => sum + group.population, 0) });
}
