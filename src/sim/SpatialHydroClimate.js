...
export const HYDROCLIMATE_POLICY = "krapp-777-branch-response-v1";
...
function responseState(globalState, latitude) {
  ...
  // A deliberately conservative hydrological sensitivity: warming raises the
  // water-cycle response while additional high-latitude ice suppresses local
  // precipitation. This is model policy, not a Krapp-derived measurement.
  const precipitationLogScale = temperatureDelta * (0.018 + (1 - polarWeight) * 0.010) - iceDelta * polarWeight * 0.55;
  const precipitationScale = clamp(Math.exp(precipitationLogScale), 0.5, 1.7);
  return { polarWeight, globalTemperatureDelta, temperatureDelta, iceDelta, precipitationScale };
}
...
function waterBalanceIndex(temperatureCelsius, precipitationMmPerYear, cloudCoverPercent) {
  ...
  const temperatureDemand = Math.exp(clamp(temperatureCelsius, -20, 35) * 0.032);
  const cloudRelief = 1 - clamp((cloudCoverPercent ?? 50) / 100, 0, 1) * 0.18;
  const atmosphericDemand = 620 * temperatureDemand * cloudRelief;
  return clamp(precipitationMmPerYear / (precipitationMmPerYear + atmosphericDemand), 0.02, 0.995);
}
...
export class SpatialHydroClimate {
  ...
  sample(globalState, latitude, longitude, spatialDetail = 0.35) {
    ...
    const response = responseState(globalState, cell.latitude);
    ...
    const cloudCoverPercent = Number.isFinite(checkpoint.cloudCoverPercent)
      ? clamp(
          checkpoint.cloudCoverPercent + Math.log(response.precipitationScale) * 18 - response.temperatureDelta * 0.65,
          0,
          100
        )
      : null;
    ...
    const result = Object.freeze({
      ...
      temperatureDelta: round(response.temperatureDelta, 4),
      precipitationScale: round(response.precipitationScale, 5),
      policy: HYDROCLIMATE_POLICY,
      epistemicStatus: globalState.elapsedYears > 0
        ? "model derived branch response from study-constrained Krapp checkpoint"
  ...
  monthlyAt(globalState, month, latitude, longitude, spatialDetail = 0.35) {
    ...
    return Object.freeze({
      ...
      monthIndex: checkpoint.monthIndex,
      latitude: cell.latitude,
      longitude: cell.longitude,
      gridSpacingDegrees: spacing,
      temperatureCelsius: round(temperatureCelsius, 3),
      ...
      epistemicStatus: globalState.elapsedYears > 0
        ? "model derived branch response from study-constrained Krapp checkpoint"
        : "study-constrained Krapp checkpoint climate"
    });
  }
