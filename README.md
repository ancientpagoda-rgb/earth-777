...
- a closed monthly soil-water budget with Priestley–Taylor/FAO radiation PET and ETOPO downhill runoff-parcel routing;
...
The spatial climate checkpoint now comes directly from the final published Krapp et al. (2021) 0.5° monthly reconstruction. Earth 777 extracts only the exact −777,000-year time slice from each of the authors’ 36 temperature, precipitation, and cloud-cover NetCDFs, verifies every source SHA-256, masks the CDO missing-data sentinel, and compacts the result into a deterministic 3.94 MB gzip layer. After the checkpoint, a CWF-driven virtual grid produces explicitly model-derived spatial temperature, precipitation, and cloud responses. A closed monthly soil-water bucket then conserves precipitation into actual evapotranspiration, runoff, and storage change; runoff parcels move downhill over ETOPO until reaching an ocean outlet or a closed-basin sink. This is not yet an upstream-accumulating river/groundwater/lake model.
...
3. **Integrated phase one:** CWF-driven branch hydroclimate, closed land-water budget, and ETOPO downhill runoff parcels; upstream river accumulation, lakes, groundwater and snow remain future work.
