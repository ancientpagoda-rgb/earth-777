...
// The curved local terrain and globe now share the same physical Earth radius in
// scene space. During the crossfade the globe is translated below the tangent
// plane, so its surface coincides with the bent terrain instead of appearing as
// a smaller sphere sitting on top of the landscape.
...
source = source.replace(surfaceModeStateMarker, [
  ...
  "globeRoot.updateMatrixWorld(true);const planetaryGlobeBaseScale=globeRoot.scale.clone(),planetaryGlobeBasePosition=globeRoot.position.clone(),planetaryGlobeBounds=new THREE.Box3().setFromObject(globeRoot),planetaryGlobeNativeRadius=Math.max(.001,planetaryGlobeBounds.getBoundingSphere(new THREE.Sphere()).radius);",
  ...
  "function alignGlobeToCurvedSurface(){const sceneScale=12/surfaceSpanKm,targetRadius=EARTH_RADIUS_KM*sceneScale,multiplier=targetRadius/planetaryGlobeNativeRadius;globeRoot.scale.copy(planetaryGlobeBaseScale).multiplyScalar(multiplier);globeRoot.position.copy(planetaryGlobeBasePosition);globeRoot.position.y-=targetRadius*FY;document.body.dataset.planetaryRadius=targetRadius.toFixed(4);return targetRadius}",
  ...
  "function applySurfaceCameraPose(t,far,planetRadius){const baseTargetY=-.25,baseZ=6.8,baseY=4.25,localRadius=Math.max(.25,Math.hypot(baseZ,baseY-baseTargetY)),globeRadius=Math.max(localRadius,planetRadius*(2.8+.35*far)),radius=mix(localRadius,globeRadius,t),targetX=surfaceFocusX*(1-t),targetY=mix(baseTargetY,-planetRadius*FY,t)+surfaceFocusY*(1-t),defaultPitch=mix(Math.atan2(baseY-baseTargetY,baseZ),.12,t),pitch=clamp(defaultPitch+surfaceOrbitPitch,-1.2,1.35),horizontal=Math.max(.05,Math.cos(pitch)*radius);camera.position.set(targetX+Math.sin(surfaceOrbitYaw)*horizontal,targetY+Math.sin(pitch)*radius,Math.cos(surfaceOrbitYaw)*horizontal);camera.up.set(0,1,0);camera.lookAt(targetX,targetY,0);if(surfaceOrbitRoll)camera.rotateZ(surfaceOrbitRoll)}",
  ...
  "function restorePlanetaryPresentation(){globeRoot.scale.copy(planetaryGlobeBaseScale);globeRoot.position.copy(planetaryGlobeBasePosition);for(const state of planetaryMaterialState){if(state.material.transparent!==state.transparent){state.material.transparent=state.transparent;state.material.needsUpdate=true}state.material.opacity=state.opacity;state.material.depthWrite=state.depthWrite}surfaceMesh.material.opacity=1;surfaceMesh.material.transparent=false;surfaceMesh.material.needsUpdate=true;waterMesh.material.opacity=activeBiomeWaterOpacity;plantMesh.material.opacity=1;animalMesh.material.opacity=1;riverLines.material.opacity=activeBiomeRiverOpacity;delete document.body.dataset.surfaceSpanKm;delete document.body.dataset.surfaceZoomBlend;delete document.body.dataset.planetaryZoom;delete document.body.dataset.surfaceCurvature;delete document.body.dataset.planetaryRadius}",
  "function applySurfaceZoomPresentation(){if(mode!=='surface')return;applyBiomeSurfaceCharacter(false);const curve=planetaryCurveBlend(),t=planetaryZoomBlend(),far=planetaryFarZoom(),local=1-t,planetRadius=alignGlobeToCurvedSurface();selectionMarker.visible=false;if(t>.001)centerGlobeOnSurface();setPlanetaryGlobeOpacity(t);surfaceGroup.visible=t<.999;surfaceMesh.material.transparent=true;surfaceMesh.material.opacity=local;waterMesh.material.opacity=activeBiomeWaterOpacity*local*(1-curve);plantMesh.material.transparent=true;plantMesh.material.opacity=local;animalMesh.material.transparent=true;animalMesh.material.opacity=local;riverLines.material.opacity=activeBiomeRiverOpacity*local;if(surfaceSpanKm>1800){plantMesh.visible=false;animalMesh.visible=false}if(surfaceSpanKm>9000)riverLines.visible=false;applySurfaceCameraPose(t,far,planetRadius);document.body.dataset.surfaceSpanKm=surfaceSpanKm.toFixed(1);document.body.dataset.surfaceZoomBlend=t.toFixed(4);document.body.dataset.surfaceCurvature=curve.toFixed(4);document.body.dataset.planetaryZoom=t>=.999?'globe':t>.001?'transition':curve>.001?'curved-surface':'surface'}"
].join('\n  '));
